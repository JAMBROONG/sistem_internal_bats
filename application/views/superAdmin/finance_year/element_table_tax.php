    ...
    <table class="table table-striped align-middle" style="font-size:12px">   
        <thead class="thead-inverse">
            ...
            <tbody>
                <?php
                $con = "text-nowrap";
                $row = $this->M_table->getByCon('jenis_pajak','jenis_pajak',"'PPh Badan'");
                ?> 
                <tr> 
                    <td><label class="checkbox text-sm btn btn-sm btn-default"><input type="checkbox" name="pakai"><span class="checkmark"></span>ya</label></td>
                    <td class="<?=$con?>"><?= $row['jenis_pajak'] ?></td>
                    <td><input type="text" name="rev" class=""></td>
...
<table class="table align-middle table-hover" style="font-size:12px">   
    <thead class="thead-inverse">
        ...
        <tbody>
            ...
            <tr > 
                <td class="p-2"><label class="checkbox text-sm btn btn-sm btn-default"><input type="checkbox" <?=$checked ?> name="pakai"><span class="checkmark"></span>ya</label></td>
                <td class="<?=$con?> p-2"><?= $row['jenis_pajak'] ?><input type="hidden" value="<?=$row['id']?>" name="id"><input type="hidden" value="<?=$row['batas_pembayaran']?>" name="batas_pembayaran"><input type="hidden" value="<?=$row['batas_pelaporan']?>" name="batas_pelaporan"></td>
                <td class="p-2"><input type="text" name="rev" value="<?=$row['rev']?>" class=""></td>
                ...
                <td class="p-2"><input type="text" name="nominal_pembayaran"  value="<?= number_format($row['nominal_pembayaran'], 0, ',', '.'); ?>" class="rupiah-input text-right"></td>
                ...
                <td class="p-2"><input type="text" name="nominal_pelaporan"value="<?= number_format($row['nominal_pelaporan'], 0, ',', '.'); ?>"  class="rupiah-input text-right"></td>
