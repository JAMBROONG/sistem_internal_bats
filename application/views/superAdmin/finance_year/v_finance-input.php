...
<body class="hold-transition sidebar-mini lyt mt-5">
    <div class="ms-5 me-5">
        <section class="content mt-4 ml-lg-5 mr-lg-5">
            <div class="main-body">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6 text-center">
                            <h1><?= $date?></h1>
                            <p>Enter financial data for the year: <?= $date?></p>
                        </div>
                        <form action="<?= base_url('SuperAdmin/finance_input_year')?>" class="form col-md-6" method="get">
                            <div class="form-group">
                                <label for="">Select Date</label>
                                <input type="hidden" name="user_id" value="<?= $user_id ?>">
                                <select class="js-example-basic-single form-control" style="width:100%;" name="date" id="years">
                                ...
                                <button type="submit" class="btn btn-success mt-2 btn-sm"><i class="fa fa-eye"></i> view data</button>
                                <button type="button" onclick="window.location = '<?=base_url('SuperAdmin/finance-year?show=y&user_id='.$user_id.'&date='.$date)?>'" class="btn btn-sm btn-secondary mt-2"><img src="https://cdn.iconscout.com/icon/premium/png-512-thumb/dashboard-126-464794.png?f=avif&w=256" class="mr-2" width="17px" alt=""> dashboard</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <a href="#perpajakan" class="btn btn-sm btn-warning  mb-2 mt-2">Lihat Data Perpajakan</a>
            <div class="card shadow-none border-0">
                <div class="card-header">
                    <div class="card-title">
                        Transkrip Laporan Keuangan  <?= $client['company'] ?></b>
                    </div>
                    <div class="card-tools">
                        <div class="card-title">Date: <b><?=$date?></b></div>
                    </div>
                </div>
                <div class="card-body table-responsive p-0">
                    <div class="d-flex justify-content-between align-items-center p-3">
                         
                        <a href="<?=base_url('SuperAdmin/finance_manage_year?user_id='.$user_id.'&date='.$date)?>" class="btn btn-sm btn-default" data-toggle="modal" data-target="#modaltambahsubakun">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <input type="hidden" value="<?= "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'] ?>" name="url">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" name="title_sub_account" id="" required aria-describedby="helpId" placeholder="">
                                            </div>
                ...
                if (empty($sptClient)) {
                    ...
                    <div class="alert border-warning mt-5" role="alert">
                        <p class="mb-0">data is not available, please set any defaults that will be used via the following <a href="<?=base_url('SuperAdmin/finance_manage_year?user_id='.$user_id.'&&date='.$date)?>" class="text-primary">link</a></p>
                    </div>
                    <?php
                } else{
                    ?>
                    <form action="<?=base_url('SuperAdmin/updateValueSptClient_year')?>" method="post" id="formAll" class="form-once-only">
                        <input type="hidden" value="<?= $date ?>" name="spt_date">
                        <input type="hidden" value="<?= $user_id ?>" name="client_id">
                        ...
                        <div class="card p-3 shadow-none border">
                            ...
                            <div class="card-body p-0">
                                <table id="table1" class="table bg-white table-hover">
                                    ...
                                    foreach ($sptClient as $rows) {
                                        ...
                                        <tr>
                                            ...
                                            <td><?=$rows['en']?></td>
                                            <td>
                                                <input type="text" name="value<?= $rows['id'] ?>" value="<?= number_format($rows['value'], 0, ',', '.'); ?>" id="rupiah<?= $rows['id'] ?>" class=" rupiah-input text-right w-100">
                                            </td>
                                            <td class="bg-warning">
                                                <input type="text" name="target<?= $rows['id'] ?>" value="<?= number_format($rows['target'], 0, ',', '.'); ?>" class=" rupiah-input text-right w-100">
                                            </td>
                                    ...
                                    if ($rows['description'] == 'AKUMULASI PENYUSUTAN') {
                                        # code...
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td>
                                                <div disabled name="value<?= $total_nilai ?>" class=" btn-dark text-right"><?=  "Rp " . number_format($total_nilai,0,',','.'); ?> </div>
                                            </td>
                                            <td>
                                                <div disabled value="total_nilai" class=" bg-warning text-right"><?=  "Rp " . number_format($total_target,0,',','.'); ?> </div>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="inputEmail3" class="col-form-label">Cash Balance In (Rp.)</label>
                                            <input type="text" class="form-control rupiah-input text-right w-100" required value="<?php if (!empty($data_tambahan[0]['value'])) { echo number_format($data_tambahan[0]['value'],0,',','.');}
                                            else{
                                                echo "0";
                                            }
                                            ?>" id="nama" name="cash_balance_in" placeholder="Cash Balance In">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="inputEmail3" class="col-form-label">Cash Balance Out (Rp.)</label>
                                            <input type="text" class="form-control rupiah-input text-right w-100" required id="email" value="<?php
                            ...
                            <table class="table align-middle  table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><?=$rows['en']?></td>
                                        <td>
                                            <input type="text" style="poi" readonly name="value<?= $rows['id'] ?>" value="<?= number_format($rows['value'],0,',','.'); ?>" class="disabled btn-secondary text-right w-100" id="rupiah<?= $rows['id'] ?>">
                                        </td>
                                        <td class="bg-warning">
                                            <input type="text" name="target<?= $rows['id'] ?>" value="<?= number_format($rows['target'], 0, ',', '.'); ?>" class=" rupiah-input text-right w-100">
                                        </td>
                                    ...
                                    <tr>
                                        ...
                                        <td><?=$rows['en']?></td>
                                        <td>
                                            <input type="text" name="value<?= $rows['id'] ?>" value="<?= number_format($rows['value'],0,',','.'); ?>" class="  rupiah-input text-right w-100" id="rupiah<?= $rows['id'] ?>">
                                        </td>
                                        <td class="bg-warning">
                                            <input type="text" name="target<?= $rows['id'] ?>" value="<?= number_format($rows['target'], 0, ',', '.'); ?>" class=" rupiah-input text-right w-100">
                                        </td>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div disabled value="total_nilai" class=" btn-dark text-right"><?=  "Rp " . number_format($total_nilai,0,',','.'); ?> </div>
                                        </td>
                                        <td>
                                            <div disabled value="total_nilai" class=" btn-warning text-right"><?=  "Rp " . number_format($total_target,0,',','.'); ?> </div>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                ...
                <div class="card p-3 shadow-none border">
                        ...
                        <table id="table3" class="table table-hover">
                            ...
                            <tbody>
                                ...
                                <?php
                                    ...
                                    $checked = "";
                                        $no++;
                                        if ($this->M_table->totalDataTableWhere('sub_spt_tahunan_client_pertahun','client_id',$user_id . ' AND spt_date = ' . $date . ' AND spt_tahunan_id = ' . $rows['id_akun_spt']) > 0) {
                                            $variable = $this->M_table->dataTableWhere('sub_spt_tahunan_client_pertahun','client_id',$user_id . ' AND spt_date = ' . $date  . ' AND spt_tahunan_id = ' . $rows['id_akun_spt']);
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" name="value_spt_akun_tambahan<?= $arrakuntambahan['id'] ?>" value="<?= number_format($arrakuntambahan['value'],0,',','.'); ?>" class="  rupiah-input text-right w-100" id="rupiah<?= $arrakuntambahan['id'] ?>">
                                    </td>
                                    <td class="bg-warning">
                                        <input type="text" name="target_spt_akun_tambahan<?= $arrakuntambahan['id'] ?>" value="<?= number_format($arrakuntambahan['target'], 0, ',', '.'); ?>" class=" rupiah-input text-right w-100">
                                    </td>
                                    <td class="bg-primary text-center">
                        ...
                        <div class="row">
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="supplier_name" id="" value="<?php
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    <small class="rounded p-1" style="background:#400030ff; color:white;">Not Due</small>
                                    <input type="text" class="form-control rupiah-input text-right mt-2" name="not_due" id="" aria-describedby="helpId" value="<?php
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    <small class="rounded p-1" style="background:#6b275aff; color:white;">
                                        < 30 Days</small>
                                            <input type="text" class="form-control rupiah-input text-right mt-2" name="30_days" value="<?php
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    <small class="rounded p-1" style="background:#ba3d5dff; color:white;">
                                        < 60 Days</small>
                                            <input type="text" class="form-control rupiah-input text-right mt-2" value="<?php
                                            ...
                                            ?>" name="60_days" id="" aria-describedby="helpId" placeholder="">
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group">
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    <small class="rounded p-1" style="background:#fe9085ff; color:white;">> 90 Days</small>
                                    <input type="text" class="form-control rupiah-input text-right mt-2" value="<?php
                    ...
                    <div class="card-footer">
                        <button type="submit" id="btn2" form="formAll" class="btn btn-sm btn-success mt-3" style="width:auto;" onclick="disableButton2(this)"><i class="fa fa-save mr-2"></i> Simpan Data Keuangan</button>
                    </div>
                </div>
    ...
    <div class="card mt-4 ml-lg-5 mr-lg-5 border-0">
        <div class="card-body p-0">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            $("#tabelQuotes").on("click", ".hapusData", function() {
                var id = $(this).data("id");
                $.ajax({
                    url: "<?= base_url('Finance/deleteQuotesYear'); ?>",
                    type: "POST",
                    data: {
                        id: id
                    },
                    success: function() {
                        $("#tabelQuotes").load("<?= base_url('SuperAdmin/finance_input_year?user_id=' . $user_id . '&date=' . $date) ?> #tabelQuotes");
                        Swal.fire({
                            position: 'center',
                            icon: 'success',
        ...
        $('#formTambahData').submit(function(e) {
            ...
            $.ajax({
                url: '<?= base_url() ?>Finance/addQuotesYear',
                type: 'POST',
                dataType: 'json',
        ...
        $(document).ready(function() {
            ...
            function load_data() {
                $.ajax({
                    url: '<?php echo base_url(); ?>Finance/getQuoteYear?user_id=<?=$user_id?>&date=<?=$date?>',
                    dataType: 'json',
