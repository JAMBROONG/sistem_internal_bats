                        ...
                        <select class="custom-select" name="title_id"  id="selectStatuses">
                            <?php
                                foreach ($this->M_table->getAll('title_quote') as $key) {
                                    if ($this->M_table->totalByCon("quote_for_client_year","client_id",$user_id ." AND date = '".$date."' AND title_id = ". $key['id']) > 0) {
                                        continue;
                                    }
                                    ?>
...
<table class="table table-borderless" id="tabelQuotes">
    <thead>
        <tr>
            <th>No.</th>
            <th>Penempatan</th>
