<table id="example" class="table table-striped table-bordered text-sm" style="width:100%">
    ...
    <tbody>
        <?php
$no =1;
foreach ($this->M_table->dataTableWhere('inventory_drung_year','user_id', $user_id . " AND date_year = '". $date ."' order by expired_date desc") as $key) {
    if (getExpired(date('d-m-Y', strtotime($key['expired_date'])))['interval'] < 28) {
        continue;
    }
    if (getExpired(date('d-m-Y', strtotime($key['expired_date'])))['interval'] > 35) {
        continue;
    }
    ?>
        <tr>
            ...
            <td><?=$key['unit']?></td>
            <td><?= date('d M Y', strtotime($key['expired_date']))?> <br> <?= getExpired(date('d-m-Y', strtotime($key['expired_date'])))['status']?></td>
            <td>
...
<script>
    document.getElementById("btn-approaching_expiration").classList.add("btn-dark");
    // document.getElementById("btn-expired").classList.remove("btn-light");
</script>