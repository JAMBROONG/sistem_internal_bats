...
class Finance_client extends CI_Controller {
	public function __construct(){
		...
		else if ($this->session->userdata('status_id') != 1) {
			redirect('Logout');
        }
        ...
        if (isset($_GET['show']) && isset($_GET['type']) && isset($_GET['date'])) {
			if ($_GET['show'] == md5('5g7d8fyyes5g7d8fy')) {
				if ($_GET['type'] == 'month' || $_GET['type'] == 'year') {
					if (preg_match('/^\d{4}$/', $_GET['date']) && $_GET['type'] == 'year' ) {
                        $user_id = $this->session->userdata('id');
                        if (!in_array($_GET['date'],array_column( $this->M_finance->getListYear($user_id), 'tahun'))) {
                        ...
                        $data['quote']                        = [];
                        foreach ($this->M_finance->getQuoteYear($user_id,$date) as $key) {
                            $data['quote'][$key['title_quote']] = $key['quotes'];
                        }
						$this->load->view('client/finance_preview/year/index',$data);
					} else if (preg_match('/^\d{4}-\d{2}$/', $_GET['date']) && $_GET['type'] == 'month') {
                        $date =$_GET['date'];
                        $user_id = $this->session->userdata('id');
                        
		                $data['list_month']       = $this->M_finance->getListYearForMonth($user_id);
                        if (!in_array($_GET['date'].'-02',array_column( $data['list_month'], 'bulan'))) {
                            return $this->load->view('errors/error_request.html');
                        }
                        $data['user_id']                      = $user_id;
                        $data['date']                         = $date;
			...
			if ($_GET['show'] == md5('5g7d8fyyes5g7d8fy')) {
				if ($_GET['type'] == 'month' || $_GET['type'] == 'year') {
					if (preg_match('/^\d{4}$/', $_GET['date'])) {
						echo 'Tahunan <br>';
					} else if (preg_match('/^\d{4}-\d{2}$/', $_GET['date'])) {
                        $date =$_GET['date'];
                        $user_id = $this->session->userdata('id');
                        $data['list_year']       = $this->M_finance->getListYear($user_id);
                        $data['komentar'] = $this->M_table->dataTableWhere('tabel_komentar_financial_dashboard','user_id',$user_id . ' order by updated_at DESC');
    ...
    public function cash_manajement_dashboard_month($user_id,$date)
    {
        ...
        // foreach ($a_turnover as $key) {
        //     echo "<br>bulan: " . $key['date'];
        //     echo "<br>AR: " . $key['value_ar'];
        //     echo "<br>AP: " . $key['value_ap'].'<br>';
        // }
        // print_r($inventory);
        // die;
        return array(
    ...
    public function cash_manajement_dashboard_year($user_id,$date)
    {
        ...
        // print_r($days_inventory_outstanding);
        // die;
        $a_turnover = [];
        $inventory = [];
        // loop untuk menampilkan bulan-bulan sebelumnya
    ...
    public function get_total_ekuitas_month($user_id,$date)
    {
        ...
        foreach ($this->M_table->get_spt_client($user_id,$date . "' AND st.category_jumlah = 'Jumlah Kewajiban dan Ekuitas") as $key) {
    ...
    public function financial_performance_dashboard_month($user_id,$date)
    {
        ...
        foreach ($this->M_finance->getListYearForMonth($user_id) as $key) {
            if (date('Y',strtotime($key['bulan'])) != date('Y',strtotime($date))) {
                 continue;
            }
...
// print_r($cash);
    ...
    public function financial_performance_dashboard_year($user_id,$date)
    {
        ...
        foreach ($this->M_finance->getListYear($user_id) as $key) {
            ...
            $roa[] = array(
                'tahun' => $key['tahun'],
                'value' => ($total_aset == 0) ? 0 : round(($laba_bersih/$total_aset) * 100)
              );
            $wcr[] = array(
                'tahun' => $key['tahun'],
                'value' => ($kewajiban_lancar == 0) ? 0 : $aset_lancar/$kewajiban_lancar
              );
            $roe[] = array(
                'tahun' => $key['tahun'],
                'value' => ($total_ekuitas == 0) ? 0 : ($laba_bersih/$total_ekuitas)*100
            );
            $der[] = array(
                'tahun' => $key['tahun'],
                'value' => ($total_ekuitas == 0) ? 0 :  $total_kewajiban/$total_ekuitas
            );
            $t_asset[] = array(
                'tahun' => $key['tahun'],
                'value' => $total_aset
    ...
    public function getStatusPembayaran($today,$deadline)
    {
        ...
        if ($interval <= 0) {
            return '<i class="text-success">Already paid</i>, past the payment limit<a class="text-danger">' . $interval . ' days </a>';
        } else {
            return '<i class="text-success">Already paid</i>, '. $interval . 'days <a class="text-success">before</a> payment deadline';
        }
    }
    ...
    public function getStatusPelaporan($today,$deadline)
    {
        ...
        $interval = $interval->format('%r%a') . "\n\n";
        if ($interval <= 0) {
            return '<i class="text-success">Already reported</i>, over the reporting line <a class="text-danger">' . $interval . ' days </a>';
        } else {
            return '<i class="text-success">Already reported</i>, '. $interval . ' days <a class="text-success">before</a> reporting limits';
        }
    }
    ...
    {
        $data['user_id'] = (!empty($_POST['user_id'])) ? $_POST['user_id'] : 0;
        $total = (!empty($_POST['total'])) ? $_POST['total']-1 : 0;
        $data['date'] = (!empty($_POST['date'])) ? $_POST['date'] : 0;
        if ($data['date'] == 0 || $data['user_id'] == 0){
            redirect('Client/financial_dashboard');
        }
        if ($this->M_table->totalByCon("tax_client_monthly","user_id",$data['user_id']." AND date = '".$data['date']."'") == 0) {
            redirect('Client/financial_dashboard');
        }
        ...
        // die;
            redirect('Finance_client/finance_month?type=month&show='.md5('5g7d8fyyes5g7d8fy').'&date='.date('Y-m', strtotime($data['date'])));
    }
    ...
    public function update_tax_year()
    {
        ...
        if ($this->M_table->totalByCon("tax_client_year","user_id",$data['user_id']." AND date = '".$data['date']."'") == 0) {
            ...
            $data['status_pembayaran']    = '<i class="text-danger">Not yet paid</i>';
            $data['nominal_pembayaran']   = (!empty($_POST['nominal_pembayaran'])) ? (int)str_replace(["Rp. ", "."], "", trim($_POST['nominal_pembayaran'])):"";
            ...
            $data['nominal_pelaporan']    = (!empty($_POST['nominal_pelaporan'])) ? (int)str_replace(["Rp. ", "."], "", trim($_POST['nominal_pelaporan'])):"";
            ...
            redirect('Finance_client/finance_month?type=year&show='.md5('5g7d8fyyes5g7d8fy').'&date='.$data['date'].'&tab=td');
    }
